...
class BeBoldPPODiscretePolicy(PPODiscretePolicy):
...
def train(config):
    ...
    # Modify worker to add BeBold intrinsic rewards
    class BeBoldPPOWorker(PPOWorker):
        ...
        def collect_batch(self) -> Tuple[dict, np.ndarray, np.ndarray]:
            ...
            for _ in range(self.n_steps):
                ...
                # Calculate BeBold intrinsic reward
                with th.no_grad():
                    state_emb, _ = self.policy.target_net.extract_features(
                        th.as_tensor(obs[None, ...]).float().to(self.device),
                        th.as_tensor(first[None, ...]).float().to(self.device),
                    )
                    pred_state_emb, _ = self.policy.predictor_net.extract_features(
                        th.as_tensor(obs[None, ...]).float().to(self.device),
                        th.as_tensor(first[None, ...]).float().to(self.device),
                    )
                ...
                rnd_error = th.norm(pred_state_emb - state_emb, dim=2, p=2)
                next_rnd_error = th.norm(
                    pred_next_state_emb - next_state_emb, dim=2, p=2
                )
                bebold_rew = th.clamp(next_rnd_error - rnd_error, min=0)
                bebold_rew = bebold_rew.cpu().numpy().squeeze(0)
                ...
                # Add into batch
                batch["bebold_rew"].append(bebold_rew)
            ...
            batch["bebold_rew"] = np.asarray(batch["bebold_rew"], dtype=np.float32)
        ...
        def process_batch(
            ...
            """
            Add BeBold intrinsic reward
            """
            intrinsic_rewards = batch["bebold_rew"]
    ...
    # Create worker
    worker = BeBoldPPOWorker(**config["worker_kwargs"])
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "run_cfg": {
            "run_id": args.run_id,
            "log_dir": f"./exps/bebold_perm_{args.ep_curiosity}/",
        ...
        "worker_kwargs": {
            ...
            "policy_fn": "__main__.BeBoldPPODiscretePolicy",
            "policy_kwargs": {
                "extractor_fn": "cnn",
                "extractor_kwargs": {
                    "input_shape": (3, 7, 7),
